...
const CyberpunkText = ({ text = 'CYBERPUNK' }) => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (Math.random() < 0.1) {
        ...
        ctx.fillText(text, dimensions.width / 2 + Math.random() * 4 - 2, dimensions.height / 2 + Math.random() * 4 - 2);
      }
  ...
  return <canvas ref={canvasRef} width={dimensions.width} height={dimensions.height} style={{ width: '100%', height: 'auto' }} />;
};
