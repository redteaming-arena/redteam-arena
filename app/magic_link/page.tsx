"use server"
import MagicLinkCard from "@/components/cards/magic-link";

export default async function MagicLink(params: any) {
  ...
  return (
    <>
      ...
      <div className="font-sans flex flex-col items-center justify-center min-h-screen ">
        ...
        <div className=" text-white text-4xl font-bold p-5 rounded-xl shadow-xl mb-10">
            {"/>"}
      </div>
        <div className="w-72 max-w-md">
          <MagicLinkCard magic_token={params.searchParams.token} />
        </div>
      </div>
    </>
  );
